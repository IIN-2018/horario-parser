...
const { htmlTest } = require('../utils/test_html');
...
const getUrlDescargaHorarioClases = async ($) => {
  try {
    ...
    const horarioSeccion = $('.elementor.elementor-1100')
      .children()
      .children()
      .children()
      .last();
    ...
    const tituloSeccionHorarioPlaificacion = horarioSeccion
      .find('h3')
      .first()
      .text()
      .trim();
    ...
    const urlHorarioDescarga = horarioSeccion
      .find('p')
      .children()
      .attr('href');

    if (!urlHorarioDescarga) {
      throw new Error(`Error al obtener el URL realizando el Web Scrapping`);
    }
